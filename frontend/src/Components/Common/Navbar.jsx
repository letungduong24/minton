  ...
  return (
    <div className='absolute z-50 w-full p-7'>
      <div className="flex justify-between items-center">
        ...
        <div className="hidden lg:block menu space-x-2 bg-white/10 rounded-2xl px-2 py-4 backdrop-blur-xs border border-white/20">
          ...
          {!user ? (
            <Link className='text-white font-medium text-lg px-2 py-3 rounded-xl hover:scale-105 transition-all duration-300 hover:bg-gray-200/30 ' to={'/auth/login'}>Đăng nhập</Link>
          ) : (
            <Link className='text-white font-medium text-lg px-2 py-3 rounded-xl hover:scale-105 transition-all duration-300 hover:bg-gray-200/30 ' to={'/auth/login'}>Tài khoản</Link>
          )}
          {user && user.role === 'admin' && (
            <Link className='text-white font-medium text-lg px-2 py-3 rounded-xl hover:scale-105 transition-all duration-300 hover:bg-gray-200/30 ' to={'/admin'}>Admin</Link>
          )}
        </div>
